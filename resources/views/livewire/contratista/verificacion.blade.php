<div class="p-4 bg-gray-100 dark:bg-gray-900 min-h-screen">
    <div class="mb-4 flex items-center gap-4">
        @if($is_from_legacy)
            <a href="{{ route('contratista.verificacion-legacy') }}" 
    ...
    <div style="display: flex; gap: 1rem; align-items: flex-start; width: 100%;">
        ...
        @if(!$is_from_legacy)
            ...
            <div style="width: 300px; flex-shrink: 0;" class="space-y-4">
            <!-- SELECCION EMPRESA -->
                    ...
                    <div class="p-2 max-h-[600px] overflow-y-auto custom-scrollbar space-y-1">
                        @forelse($periodos as $p)
                            @php
                                ...
                                $isBloqueado = $p['fuera_vigencia'] || $p['excluido'];
                                $isActivo = $periodoActual && $periodoActual['mes'] == $p['mes'] && $periodoActual['anio'] == $p['anio'];
                                
                                $bgClass = $isEmitido ? 'bg-[#003a5c]' : ($isEnviado ? 'bg-[#3b82f6]' : ($isInProg ? 'bg-[#8ed973]' : 'bg-white dark:bg-gray-800'));
                                $borderCls = $isActivo ? 'border-l-4 border-l-[#004b75] dark:border-l-cyan-500 shadow-md ring-1 ring-[#004b75]/20 dark:ring-cyan-500/20' 
                                                       : 'border border-gray-200 dark:border-gray-700 hover:border-[#004b75]/50 dark:hover:border-cyan-500/50 hover:shadow-sm opacity-90 hover:opacity-100';
                                
                                $textClass = ($isEmitido || $isEnviado) ? 'text-white' : ($isInProg ? 'text-[#003a5c]' : 'text-[#004b75] dark:text-cyan-400');
                            ...
                            <button wire:click="seleccionarPeriodo({{ $p['anio'] }}, {{ $p['mes'] }})"
                                ...
                                <div class="flex justify-between items-start mb-2 group-hover:relative">
                                    <div class="flex flex-col">
                                        <h4 class="text-[11px] font-black uppercase {{ $textClass }} tracking-tight leading-none mb-1">
                                            REMUNERACIONES {{ $p['nombre_mes'] }} <br> <span class="{{ $subTextClass }} text-[13px]">{{ $p['anio'] }}</span>
                                        </h4>
                                        <div class="flex items-center gap-1.5 mt-0.5">
                                            @if($p['fuera_vigencia'] || $p['excluido'])
                                                <span class="text-[8px] font-bold tracking-wider uppercase text-red-500 flex items-center gap-1"><span class="w-1.5 h-1.5 rounded-full bg-red-500"></span> {{ $p['excluido'] ? 'BLOQUEADO POR MANDANTE' : 'FUERA DE VIGENCIA' }}</span>
                                            @elseif($p['estado'] === 'PENDIENTE')
                                                <span class="text-[8px] text-gray-500 dark:text-gray-400 font-black bg-gray-100 dark:bg-gray-600 px-2 py-0.5 rounded shadow-sm">○ PENDIENTE</span>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-1">
                                        ...
                                        @elseif($p['estado'] == 'ENVIADO')
                                            <span class="text-[8px] text-green-700 dark:text-green-300 font-black bg-green-100 dark:bg-green-900/50 px-2 py-0.5 rounded shadow-sm">✓ ENVIADO</span>
                                        @elseif($p['estado'] == 'EN PROGRESO')
                                            <span class="text-[8px] text-blue-700 dark:text-blue-300 font-black bg-blue-100 dark:bg-blue-900/50 px-2 py-0.5 rounded shadow-sm">◐ EN PROGRESO</span>
                                        @else
                                            <span class="text-[8px] text-gray-500 dark:text-gray-400 font-black bg-gray-100 dark:bg-gray-600 px-2 py-0.5 rounded shadow-sm">○ PENDIENTE</span>
                                        @endif
                                    </div>
                                ...
                                <div class="mb-2">
                                    ...
                                    @elseif($p['estado_plazo'] == 'FUERA_PLAZO')
                                        <span class="text-[8px] text-amber-600 dark:text-amber-400 font-black bg-amber-50 dark:bg-amber-900/50 px-2 py-0.5 rounded">⚠ ESTÁS FUERA DE PLAZO</span>
                                    @elseif($p['estado_plazo'] == 'FUTURO')
                                        <span class="text-[8px] text-blue-600 dark:text-blue-400 font-black bg-blue-50 dark:bg-blue-900/50 px-2 py-0.5 rounded">● PRÓXIMAMENTE</span>
                                    @elseif($p['estado_plazo'] == 'VENCIDO')
                                        <span class="text-[8px] text-red-600 dark:text-red-400 font-black bg-red-50 dark:bg-red-900/50 px-2 py-0.5 rounded">✖ ESTÁS FUERA DE PLAZO</span>
                                    @endif
            ...
            @endif {{-- /periodos section --}}
            </div> {{-- /left panel 300px --}}
        @endif {{-- /!is_from_legacy --}}

        <!-- PANEL DERECHO: siempre dentro del flex -->
        <div style="flex: 1; min-width: 0;">
            @if($mes_seleccionado)
                        ...
                        <div class="flex justify-between items-center px-4 py-2 border-b border-white/10">
                            ...
                            @elseif($carpeta_actual && $carpeta_actual->estado !== 'ENVIADO')
                                ...
                                <button wire:click="abrirModalConfirmacion" 
                                        @if($errorMsg || $faltantesCount > 0) disabled title="{{ $errorMsg ?? 'Faltan documentos obligatorios' }}" class="bg-gray-400 text-white font-black text-[11px] px-5 py-2 rounded shadow opacity-60 cursor-not-allowed uppercase tracking-wider"
                                        @else class="bg-green-500 hover:bg-green-600 text-white font-black text-[11px] px-5 py-2 rounded shadow-lg transition-all uppercase tracking-wider" @endif>
                                    ✓ ENVIAR PERIODO
                                </button>
                            @elseif($carpeta_actual && $carpeta_actual->estado === 'ENVIADO')
                    ...
                    @php
                        ...
                        $showContent = ($carpeta_actual->estado_revision !== 'EMITIDO' || $hasIncidences);
                    @endphp
                            ...
                            <tbody>
                                @foreach($requisitos as $clasNombre => $reqs)
                                    ...
                                    <tr class="bg-gray-100 dark:bg-gray-700 border-b border-gray-300 dark:border-gray-600">
                                        <td colspan="2" class="px-4 py-1.5 text-[10px] font-black text-[#004b75] dark:text-blue-300 uppercase tracking-widest italic">
                                            {{ $clasNombre }}
                                        </td>
                                    </tr>
                                ...
                                <table class="min-w-full border-collapse">
                                    <thead class="bg-gray-100 dark:bg-gray-700">
                                        <tr>
                                            ...
                                            <th class="px-3 py-2 text-center text-[9px] font-black text-gray-600 dark:text-gray-300 uppercase tracking-widest border-b border-gray-200 dark:border-gray-600">Estado / Acción</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @forelse($trabajadoresVinculados as $tv)
                                            @php
                                                $fiIngreso = $tv->vinculacion->fecha_ingreso_vinculacion ?? null;
                                                $esNuevo   = $fiIngreso && \Carbon\Carbon::parse($fiIngreso)->between($pStart, $pEnd);
                                                $estadoActual = $tv->estado_revision;
                                                // Retrocompatibilidad: tratar BAJA_MANDANTE igual que CESACION_PRINCIPAL
                                            ...
                                            <tr class="{{ $loop->even ? 'bg-gray-50 dark:bg-gray-700' : 'bg-white dark:bg-gray-800' }} hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors border-b border-gray-100 dark:border-gray-700 {{ $tv->tipo_registro === 'ARRASTRE' ? 'border-l-4 border-l-orange-400' : ($esNuevo ? 'border-l-4 border-l-emerald-400' : '') }}">
                                                ...
                                                <td class="px-3 py-2">
                                                    <div class="flex flex-col">
                                                        <span class="text-[10px] font-black text-gray-800 dark:text-gray-200">{{ $tv->vinculacion->trabajador->rut ?? '-' }}</span>
                                                ...
                                                <td class="px-3 py-2 text-[11px] font-bold text-gray-900 dark:text-white uppercase">
                                                    {{ $tv->vinculacion->trabajador->nombre_completo ?? '-' }}
                                                </td>
                                                {{-- CARGO --}}
                                                <td class="px-3 py-2 text-[10px] text-gray-600 dark:text-gray-400">
                                                    {{ $tv->vinculacion->cargoMandante->nombre_cargo ?? 'Sin cargo' }}
                                                </td>
                                                {{-- F. INGRESO --}}
                                                ...
                                                {{-- F. CONTRATO --}}
                                                <td class="px-3 py-2 text-center text-[9px] font-bold text-gray-700 dark:text-gray-300">
                                                    {{ $tv->vinculacion->fecha_contrato ? $tv->vinculacion->fecha_contrato->format('d/m/Y') : '-' }}
                                                </td>
                                                {{-- NUEVO --}}
                                                ...
                                                <td class="px-3 py-2">
                                                    @php
                                                        ...
                                                        if (in_array($estadoActual, ['FINIQUITADO', 'CESACION_PRINCIPAL', 'RECONOCIMIENTO_ANTIGUEDAD'])) {
                                                            // Buscar si el trabajador tiene la misma vinculación en estado emitido/enviado en el mismo mes
                                                            $vincIdsDelTrabajador = \App\Models\TrabajadorVinculacion::where('trabajador_id', $tv->vinculacion->trabajador_id)->pluck('id');
                                                            $desvinculacionBloqueada = \App\Models\CarpetaVerificacion::join('carpetas_verificacion_trabajadores', 'carpetas_verificacion.id', '=', 'carpetas_verificacion_trabajadores.carpeta_verificacion_id')
                                                                ->where('carpetas_verificacion.id', '!=', $carpeta_actual->id)
                                                                ->where('carpetas_verificacion.anio', $carpeta_actual->anio)
                                                                ->where('carpetas_verificacion.mes', $carpeta_actual->mes)
                                                                ->whereIn('carpetas_verificacion.estado', ['ENVIADO', 'EN PROGRESO']) // u otro estado final
                                                    ...
                                                    <div class="flex flex-col gap-1 items-center">
                                                        <select
                                                            ...
                                                            @if($estadoActual === 'PENDIENTE') bg-amber-50 text-amber-700 border-amber-200
                    ...
                    @else
                        <!-- MENSAJE PERIODO FINALIZADO SIN INCIDENCIAS -->
                        <div class="p-20 flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-800/50">
                            <div class="text-6xl mb-4">📜</div>
                            <h3 class="text-lg font-black text-purple-800 dark:text-purple-400 uppercase tracking-tighter">Certificación Finalizada</h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400 font-bold uppercase mt-2 text-center max-w-md">
                                Este periodo ha sido emitido exitosamente sin observaciones ni contingencias pendientes.
                            </p>
                            <div class="mt-8">
                                <a href="{{ route('verificacion.certificado.visor', $carpeta_actual->id) }}" target="_blank" class="bg-purple-600 hover:bg-purple-700 text-white font-black text-sm px-8 py-4 rounded-2xl shadow-xl transition-all transform hover:scale-105 uppercase tracking-widest flex items-center gap-3">
                    ...
                    <!-- BOTON ENVIAR AL FINAL -->
                        ...
                        @elseif($carpeta_actual && $carpeta_actual->estado !== 'ENVIADO')
                            ...
                            <div class="p-6 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-700 flex flex-col items-center justify-center">
                                ...
                                @endif
                                <button wire:click="abrirModalConfirmacion" 
                                        @if($errorMsg || !empty($obligatoriosFaltantes)) disabled title="{{ $errorMsg ?? 'Faltan documentos obligatorios' }}" class="bg-gray-400 text-white font-black text-xs px-10 py-3 rounded shadow opacity-60 cursor-not-allowed uppercase tracking-widest flex items-center gap-3"
                                        @else class="bg-green-600 hover:bg-green-700 text-white font-black text-xs px-10 py-3 rounded shadow-lg transition-all transform hover:scale-105 uppercase tracking-widest flex items-center gap-3" @endif>
            ...
            @else
                ...
                <div style="min-height: 400px; padding: 2rem;" class="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 border-dashed flex flex-col items-center justify-center opacity-30">
    ...
    @if($modal_confirmacion_visible)
        <div class="fixed inset-0 z-[999] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-[#004b75] rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border-2 border-white/20">
                    <div class="p-8">
