        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($maquinarias as $maquinaria)
                    @php
                        $acceso = $maquinaria->determinarAccesoHabilitado($mandanteId, $this->uoId);
                        $cumplimiento = $maquinaria->calcularPorcentajeCumplimiento($mandanteId, $this->uoId);
                    @endphp
                    <tr class="table-row-hover">
                        <td class="table-cell font-mono text-center">{{ ($maquinarias->currentPage() - 1) * $maquinarias->perPage() + $loop->iteration }}</td>
                        ...
                        <td class="table-cell text-center">
                            ...
                            @if($acceso['es_excepcion'])
                                <button wire:click="$parent.revertirAnulacionManual({{ $maquinaria->id }}, '{{ addslashes(get_class($maquinaria)) }}')" 
                ...
                @empty
                    <tr><td colspan="7" class="table-cell text-center">No se encontraron maquinarias en este contexto.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
