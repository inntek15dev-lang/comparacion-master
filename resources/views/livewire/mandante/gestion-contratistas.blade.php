{{-- ============================ INICIO MODIFICACIÓN 1.1 (Ancho de página) ============================ --}}
{{-- Se envuelve todo en un div que ocupa el ancho completo del contenedor del layout --}}
<div class="w-full">
...
{{-- ============================= FIN MODIFICACIÓN 1.1 ============================== --}}
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-4 sm:space-y-0">
            <div class="w-full sm:w-auto flex-grow">
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Buscar por Razón Social, RUT/NIT/RUC/CNPJ, Nombre Comercial o Admin..."
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-2 sm:space-y-0 w-full sm:w-auto sm:ml-4">
                <div class="flex items-center space-x-4">
                    <div>
                        <label for="filtroTipo" class="label-form text-xs">Tipo</label>
                        <select wire:model.live="filtroTipo" id="filtroTipo" class="input-field input-field-sm">
                            <option value="todos">Todos</option>
                            <option value="Contratista">Contratistas</option>
                            <option value="Subcontratista">Sub-Contratistas</option>
                        </select>
                    </div>
                    <div>
                        <label for="filtroEstado" class="label-form text-xs">Estado</label>
                        <select wire:model.live="filtroEstado" id="filtroEstado" class="input-field input-field-sm">
                            <option value="todos">Todos</option>
                    ...
                    <div>
                        <label for="filtroAcredita" class="label-form text-xs">Acredita</label>
                        <select wire:model.live="filtroAcredita" id="filtroAcredita" class="input-field input-field-sm">
                            <option value="todos">Todos</option>
                    ...
                    <div>
                        <label for="filtroVerifica" class="label-form text-xs">Verifica</label>
                        <select wire:model.live="filtroVerifica" id="filtroVerifica" class="input-field input-field-sm">
                            <option value="todos">Todos</option>
                            <option value="si">SÍ</option>
                            <option value="no">NO</option>
                        </select>
                    ...
                    <div>
                        <label for="filtroContrato" class="label-form text-xs">N° Contrato</label>
                        <input type="text" wire:model.live.debounce.300ms="filtroContrato" id="filtroContrato" 
                               class="input-field input-field-sm" placeholder="Buscar contrato...">
                    </div>
                    <div>
                        <label for="filtroTipoContrato" class="label-form text-xs">Tipo Contrato</label>
                        <select wire:model.live="filtroTipoContrato" id="filtroTipoContrato" class="input-field input-field-sm">
                            <option value="todos">Todos</option>
                    ...
                    <div class="self-end">
                        <button wire:click="limpiarFiltros" class="btn-secondary py-2 text-sm">
                            <x-icons.x-circle class="h-4 w-4 inline-block mr-1"/>
                            Limpiar
                        </button>
                    </div>
                </div>
                <div class="w-full sm:w-auto">
                    @if(!$esSoloLectura)
                    <button wire:click="create()" class="btn-primary w-full">
        ...
        <div class="flex flex-wrap gap-4 items-center text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded-lg mb-4">
            <span class="font-semibold text-gray-700 dark:text-gray-300">Excluir columnas:</span>
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" wire:model.live="columnasExcluidas" value="tipo" class="rounded text-blue-600">
                <span class="ml-1 text-gray-600 dark:text-gray-400">Tipo</span>
            </label>
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" wire:model.live="columnasExcluidas" value="admin" class="rounded text-blue-600">
                <span class="ml-1 text-gray-600 dark:text-gray-400">Admin. Plataforma</span>
            </label>
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" wire:model.live="columnasExcluidas" value="lugar" class="rounded text-blue-600">
                <span class="ml-1 text-gray-600 dark:text-gray-400">Lugares Trabajo</span>
            </label>
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" wire:model.live="columnasExcluidas" value="uo" class="rounded text-blue-600">
                <span class="ml-1 text-gray-600 dark:text-gray-400">U.O.</span>
            </label>
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" wire:model.live="columnasExcluidas" value="rut" class="rounded text-blue-600">
                <span class="ml-1 text-gray-600 dark:text-gray-400">RUT/NIT...</span>
            </label>
        </div>
                ...
                <table class="min-w-full border-collapse">
                    <thead class="bg-gray-100 dark:bg-gray-700 sticky top-0 z-10 shadow-sm">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider border border-gray-400 dark:border-gray-500">RUT/NIT/RUC/CNPJ</th>
                            @endunless
                            ...
                            <th scope="col" class="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider border border-gray-400 dark:border-gray-500">Lugares de Trabajo</th>
                            @endunless
                            ...
                            @endunless
                            <th scope="col" class="px-2 py-3 text-center text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider border border-gray-400 dark:border-gray-500">Acredita</th>
                            <th scope="col" class="px-2 py-3 text-center text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider border border-gray-400 dark:border-gray-500">Verifica</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse ($solicitudes as $solicitud)
                            ...
                            <tr class="{{ $fondoClase }} hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors duration-150">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm font-mono font-semibold text-teal-700 dark:text-teal-300 border border-gray-400 dark:border-gray-500">{{ $solicitud->pivot_sap ?? '-' }}</td>
                                ...
                                @unless(in_array('rut', $columnasExcluidas))
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300 border border-gray-400 dark:border-gray-500">{{ $solicitud->rut }}</td>
                                @endunless
                                @unless(in_array('tipo', $columnasExcluidas))
                                ...
                                @unless(in_array('admin', $columnasExcluidas))
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300 border border-gray-400 dark:border-gray-500">
                                    {{ $solicitud->admin_name ?? 'No asignado' }}
                                    @if($solicitud->admin_email) <br><small class="text-gray-500 dark:text-gray-400">{{ $solicitud->admin_email }}</small> @endif
                                </td>
                                @endunless
                                @unless(in_array('lugar', $columnasExcluidas))
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300 border border-gray-400 dark:border-gray-500">
                                    @if($solicitud->dep_row_id)
                                        ...
                                        @endphp
                                        <div class="text-xs">{{ $dep->nombre_jerarquico ?? $solicitud->dep_nombre ?? 'N/D' }}</div>
                                    @else
                                ...
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300 border border-gray-400 dark:border-gray-500">
                                    @if($solicitud->uo_row_id)
                                        ...
                                        @endphp
                                        <div class="text-xs font-semibold">{{ $uo->nombre_jerarquico ?? $solicitud->uo_nombre ?? 'U.O. No Encontrada' }}</div>
                                    @else
                                    ...
                                    @if($solicitud->pivot_id)
                                        <div class="flex flex-col items-center gap-0.5">
                                            ...
                                            <span wire:click="toggleAcreditaUo({{ $solicitud->pivot_id }})"
                                                  class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full cursor-pointer {{ $solicitud->pivot_acredita ? 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-100' }}">
                                                {{ $solicitud->pivot_acredita ? 'SÍ' : 'NO' }}
                                            </span>
                                            @else
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full cursor-not-allowed {{ $solicitud->pivot_acredita ? 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-100' }}">
                                                {{ $solicitud->pivot_acredita ? 'SÍ' : 'NO' }}
                                            </span>
                                            @endif
                                            @if($solicitud->pivot_fecha_inicio_acredita || $solicitud->pivot_fecha_fin_acredita)
                                                <div class="text-[10px] text-gray-600 dark:text-gray-300 leading-tight font-medium">
                                ...
                                <td class="px-2 py-2 text-center text-sm border border-gray-400 dark:border-gray-500">
                                    @if($solicitud->pivot_id)
                                        <div class="flex flex-col items-center gap-0.5">
                                            ...
                                            @else
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full cursor-not-allowed {{ $solicitud->pivot_verifica ? 'bg-purple-100 text-purple-800 dark:bg-purple-700 dark:text-purple-100' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-100' }}">
                                                {{ $solicitud->pivot_verifica ? 'SÍ' : 'NO' }}
                                            </span>
                                            @endif
                                            @if($solicitud->pivot_fecha_inicio_verifica || $solicitud->pivot_fecha_fin_verifica)
                                                <div class="text-[10px] text-gray-600 dark:text-gray-300 leading-tight font-medium">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-center text-sm border border-gray-400 dark:border-gray-500">
                                    @if(!$esSoloLectura)
                                    <span wire:click="toggleActive({{ $solicitud->contratista_id }})"
                                          wire:confirm="¿Está seguro de {{ $solicitud->is_active ? 'desactivar' : 'activar' }} a {{ $solicitud->razon_social }}?"
                                          class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full cursor-pointer {{ $solicitud->is_active ? 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100' : 'bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-100' }}">
                                        {{ $solicitud->is_active ? 'Activo' : 'Inactivo' }}
                                    </span>
                                    @else
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full cursor-not-allowed {{ $solicitud->is_active ? 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100' : 'bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-100' }}">
                                        {{ $solicitud->is_active ? 'Activo' : 'Inactivo' }}
                                    </span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap text-center text-sm font-medium border border-gray-400 dark:border-gray-500">
                                    <div class="flex flex-col items-start space-y-1">
                                        <button wire:click="edit({{ $solicitud->contratista_id }})" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-200 text-xs">{{ $esSoloLectura ? 'Ver Ficha' : 'Editar Ficha' }}</button>
                                        @if($solicitud->pivot_id)
                                        <button wire:click="abrirModalVinculaciones({{ $solicitud->contratista_id }}, {{ $solicitud->pivot_id }})" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-200 text-xs font-bold">Gestionar Vinculación</button>
                                        @endif
                                        @if(!$esSoloLectura)
                                        <button wire:click="abrirModalVinculaciones({{ $solicitud->contratista_id }})" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-200 text-[10px] italic">+ Crear Nueva Vinculación</button>
                                        @endif
                                    </div>
                                </td>
                            </tr>
                        ...
                        @empty
                            <tr>
                            <td colspan="14" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 text-center border border-gray-400 dark:border-gray-500">No se encontraron empresas contratistas que coincidan con los filtros aplicados.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        @if ($isOpen)
            <div class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title-contratista" role="dialog" aria-modal="true">
                <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    {{-- ============================ INICIO MODIFICACIÓN 1.2 (Ancho de modal) ============================ --}}
                    {{-- Se aumenta el ancho del modal de 4xl a 6xl para dar más espacio al contenido --}}
                                ...
                                <div class="sm:flex sm:items-start mb-4">
                                    ...
                                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                        <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="modal-title-contratista">
                                            {{ $contratistaId ? ($esSoloLectura ? 'Ver' : 'Editar') : 'Crear Nueva' }} Empresa Contratista
                                        </h3>
                                    </div>
                                </div>

                                <div class="space-y-6" @if($esSoloLectura) style="pointer-events: none; opacity: 0.8;" @endif>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                                            @if(!$contratistaId)
                                                <div class="md:col-span-2">
                                                    <label for="mandante_id_vinculacion" class="label-form">Vincular al Mandante <span class="text-red-500">*</span></label>
    ...
    @if ($showModalVinculaciones)
        <div class="fixed z-[60] inset-0 overflow-y-auto" aria-labelledby="modal-title-vinculaciones" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left shadow-xl transform transition-all sm:my-8 sm:align-middle sm:w-full border-t-8 border-indigo-600" style="max-width: 95vw;">
                    <form wire:submit.prevent="guardarVinculaciones">
                        <div class="bg-white dark:bg-gray-800 px-6 pt-5 pb-4 sm:p-8">
                            <div class="flex justify-between items-start mb-6 border-b pb-4 dark:border-gray-700">
                                <div>
                                    <h3 class="text-2xl leading-6 font-black text-gray-900 dark:text-white flex items-center" id="modal-title-vinculaciones">
                                        <x-icons.link class="h-8 w-8 text-indigo-600 mr-3"/>
                            ...
                            <div class="mt-4">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" wire:key="vinc-form-{{ $index }}">
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <select wire:model.live="vinculacionesTemp.{{ $index }}.mandante_id" class="input-field-sm w-full opacity-70 bg-gray-50 cursor-not-allowed" disabled>
                                                    <option value="">-- Seleccionar --</option>
                                                    @foreach($mandantesDisponibles as $m)
                                                        <option value="{{ $m->id }}">{{ $m->razon_social }}</option>
                                                    @endforeach
                                                </select>
                                                <p class="text-[10px] text-gray-500 mt-1 italic">La principal no se puede cambiar.</p>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <select wire:model.live="vinculacionesTemp.{{ $index }}.dependencia_id" class="input-field-sm w-full">
                                                    <option value="">-- Ninguno --</option>
                                                    @if($row['mandante_id'])
                                                        @php $mSel = $mandantesDisponibles->find($row['mandante_id']); @endphp
                                                        @if($mSel)
                                            ...
                                            <div>
                                                ...
                                                <select wire:model.live="vinculacionesTemp.{{ $index }}.unidad_organizacional_mandante_id" class="input-field-sm w-full">
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div class="grid grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <input type="text" wire:model.lazy="vinculacionesTemp.{{ $index }}.id_registro" class="input-field-sm w-full" placeholder="Automático" @if(!empty($row['generar_id_registro_auto']) || !empty($row['id_registro_readonly'])) disabled @endif>
                                                    <label class="flex items-center mt-1 cursor-pointer">
                                                        <input type="checkbox" wire:model.live="vinculacionesTemp.{{ $index }}.generar_id_registro_auto" class="form-checkbox h-3 w-3" @if(!empty($row['id_registro_readonly'])) disabled @endif>
                                                ...
                                                <div>
                                                    ...
                                                    <input type="text" wire:model.lazy="vinculacionesTemp.{{ $index }}.sap" class="input-field-sm w-full font-mono uppercase bg-gray-50 opacity-70" disabled>
                                                </div>
                                            </div>
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                @else
                                                <select wire:model.lazy="vinculacionesTemp.{{ $index }}.numero_contrato" class="input-field-sm w-full @if($row['id']) bg-gray-50 opacity-70 @endif" @if($row['id']) disabled @endif>
                                            ...
                                            <div class="grid grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <select wire:model.lazy="vinculacionesTemp.{{ $index }}.tipo_contrato_id" class="input-field-sm w-full @if($row['id']) bg-gray-50 opacity-70 @endif" @if($row['id']) disabled @endif>
                                                        <option value="">-- Seleccionar --</option>
                                                        @foreach($tiposContrato as $tc)
                                                            <option value="{{ $tc->id }}">{{ $tc->nombre }}</option>
                                                        @endforeach
                                ...
                                <p class="text-[10px] text-gray-500 dark:text-gray-400 mt-4 px-1 flex items-center">
                                    ...
                                    <span class="font-bold">Ayuda:</span> Para completar la vinculación, asigne al menos un Lugar de Trabajo o una U.O.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-100 dark:bg-gray-700/50 px-6 py-4 flex flex-col sm:flex-row justify-end items-center gap-4">
                            <div class="flex gap-3 w-full sm:w-auto">
                                ...
                                <button type="submit" class="btn-primary flex-1 sm:flex-initial !bg-blue-600 hover:!bg-blue-700">
    ...
    @if($showModalGestionCargos)
        <div class="fixed z-[80] inset-0 overflow-y-auto" aria-labelledby="modal-title-cargos" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl sm:w-full border-t-4 border-indigo-600">
                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6">
                        ...
                        <div class="mb-4">
                            <p class="text-xs text-gray-600 dark:text-gray-400 italic">
                                @if($isSubcontractorMode)
                                    <strong>Modo Sólo Lectura:</strong> Los cargos y cuotas asignados a este subcontratista se calculan y heredan directamente de la configuración autorizada a la contratista principal equivalente.
                                @else
                                        ...
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 font-semibold" wire:key="cargo-modal-{{ $cargo->id }}">
                                            ...
                                            <td class="px-2 py-2 text-center">
                                                <input type="number" 
                                                       ...
                                                       placeholder="{{ $isSubcontractorMode ? 'Padre Compartida' : 'Sin límite' }}"
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse space-x-reverse space-x-2">
                        ...
                        @else
                        <button type="button" wire:click="$set('showModalGestionCargos', false)" class="btn-secondary w-full sm:w-auto text-center">
                            Cerrar (Modo Lectura)
                        </button>
                        @endif
                    </div>
