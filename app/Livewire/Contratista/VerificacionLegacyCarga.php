...
class VerificacionLegacyCarga extends Component
{
    use WithFileUploads;
    ...
    public $is_from_legacy = true; // Siempre true: este módulo es exclusivo del Legacy
    ...
    public function seleccionarPeriodo($anio, $mes)
    {
        ...
        // ================================================================
        // GUARDIA #4: SIN TRABAJADORES VINCULADOS (Escudo en Carga)
        // Bloquea el inicio del período si no hay trabajadores activos
        // en la combinación exacta UO + Dependencia de la vinculación.
        // Consistente con inicializarNominaVerificada() que usa los mismos filtros.
        // ================================================================
        $carpetaExistente = CarpetaVerificacion::where('contratista_unidad_organizacional_id', $this->vinculacion_seleccionada_id)
            ->where('anio', $anio)
            ->where('mes', $mes)
            ->first();

        if (!$carpetaExistente) {
            // Solo verificamos si el período NO existe aún (iniciación nueva)
            $pStart = Carbon::create($anio, $mes, 1)->startOfMonth();
            $pEnd   = $pStart->copy()->endOfMonth();

            $tieneTrabajadores = \App\Models\TrabajadorVinculacion::whereHas('trabajador', function ($q) use ($vinculacion) {
            ...
            if (!$tieneTrabajadores) {
                session()->flash('upload_error', '❌ Sin trabajadores vinculados, no puede iniciar periodo.');
                $this->mes_seleccionado = null;
                $this->carpeta_actual = null;
                return;
            }
    ...
    public function subirArchivos($requisitoId)
    {
        ...
        try {
            ...
            if (isset($this->archivos[$requisitoId])) {
                foreach ($this->archivos[$requisitoId] as $file) {
                    $this->validateSecureFile($file, 'verificacion', 'VERIFICACION_LEGACY');
                }
            }
            throw $e;
        }
        ...
        foreach ($this->archivos[$requisitoId] as $file) {
            ...
            $storageResult = $this->encryptAndStoreFile($file, 'verificacion/' . $this->carpeta_actual->id, 'VERIFICACION_LEGACY');
            
            $nuevoDocVerif = DocumentoVerificacion::create([
            ...
            \App\Services\AuditService::log(
                'carga-verificacion',
                "Carga de Verificación: [" . ($requisito->nombre ?? 'N/A') . "] para el periodo [" . $this->getNombrePeriodo($this->carpeta_actual->mes, $this->carpeta_actual->anio) . "] - Contratista: [" . ($vinculacion->contratista->razon_social ?? 'N/A') . "]. Archivo: " . $file->getClientOriginalName(),
                [
                    'documento_verificacion_id' => $nuevoDocVerif->id,
                    'carpeta_id' => $this->carpeta_actual->id,
                    'requisito_id' => $requisitoId,
                    'contratista_id' => $vinculacion->contratista_id,
                    'periodo' => $this->getNombrePeriodo($this->carpeta_actual->mes, $this->carpeta_actual->anio),
    ...
    public function eliminarDocumento($docId)
    {
        ...
        $doc = DocumentoVerificacion::with('requisito')->find($docId);
        if ($doc) {
            $nombreDoc = $doc->requisito->nombre ?? 'N/A';
            ...
            // AUDITORÍA DETALLADA
            \App\Services\AuditService::log(
                'eliminacion-verificacion',
                "ELIMINACIÓN: Documento de Verificación: [" . $nombreDoc . "] del periodo [" . $this->getNombrePeriodo($this->carpeta_actual->mes, $this->carpeta_actual->anio) . "]. Archivo: " . $nombreOriginal,
                [
                    'carpeta_id' => $this->carpeta_actual->id,
                    'periodo' => $this->getNombrePeriodo($this->carpeta_actual->mes, $this->carpeta_actual->anio),
                    'archivo_eliminado' => $nombreOriginal
                ]
            );
        }
    ...
    public function enviarPeriodo()
    {
        ...
        // AUDITORÍA DETALLADA DE ENVÍO
        \App\Services\AuditService::log(
            'envio-verificacion',
            "ENVÍO DE PERIODO: [" . $this->getNombrePeriodo($this->carpeta_actual->mes, $this->carpeta_actual->anio) . "] - Contratista: [" . ($vinculacion->contratista->razon_social ?? 'N/A') . "] - Tipo: [" . $tipoEnvio . "]",
            [
                'carpeta_id' => $this->carpeta_actual->id,
                'contratista_id' => $vinculacion->contratista_id,
                'periodo' => $this->getNombrePeriodo($this->carpeta_actual->mes, $this->carpeta_actual->anio),
    ...
    public function render()
    {
        ...
        return view('livewire.contratista.verificacion-legacy-carga', [
    ...
    public function inicializarNominaVerificada($carpeta)
    {
        ...
        $carpeta->load('trabajadoresVerificados.vinculacion');
        foreach ($carpeta->trabajadoresVerificados as $reg) {
            $v = $reg->vinculacion;
            if (!$v) continue;

            $carpetaDate = \Carbon\Carbon::create($carpeta->anio, $carpeta->mes, 1)->startOfMonth();
            $referenciaDate = $v->fecha_finiquito ?: $v->fecha_desactivacion ?: now();
            $refCarbon = \Carbon\Carbon::parse($referenciaDate);
            $refId = ($refCarbon->year * 100) + $refCarbon->month;
            $folderId = ($carpeta->anio * 100) + $carpeta->mes;

            // SANITIZACIÓN: 
            // 1. Si la vinculación está ACTIVA en maestro, pero la nómina dice lo contrario -> Limpiamos (Reactivación fluida)
            if ($v->is_active && $reg->estado_revision !== 'PENDIENTE' && $reg->estado_revision !== 'ACTIVO') {
                $reg->update(['estado_revision' => 'PENDIENTE']);
                continue;
            }

            // 2. Si la vinculación está INACTIVA, forzamos el estado del maestro si el periodo corresponde.
            if (!$v->is_active) {
                // Si el periodo de la carpeta es ESTRICTAMENTE POSTERIOR al mes de finiquito/desactivación
                // -> ELIMINAMOS de esta nómina (Limpieza de arrastre futura)
                if ($folderId > $refId) {
                    $reg->delete();
                    continue;
                }

                $estadoMaestro = 'PENDIENTE';
                if (in_array($v->motivo_desactivacion, ['FINIQUITADO', 'CESACION_PRINCIPAL', 'RECONOCIMIENTO_ANTIGUEDAD'])) {
                    $estadoMaestro = $v->motivo_desactivacion;
                } elseif (str_contains($v->motivo_desactivacion ?? '', 'PRESENTE EN OTRA')) {
                    $estadoMaestro = 'PRESENTE_OTRA_VINCULACION';
                }

                if ($folderId == $refId && $reg->estado_revision !== $estadoMaestro) {
                    $reg->update(['estado_revision' => $estadoMaestro]);
                } elseif ($folderId < $refId && $reg->estado_revision !== 'PENDIENTE' && $reg->estado_revision === 'PRESENTE_OTRA_VINCULACION') {
                    // Escudo para duplicidad en el pasado
                    $reg->update(['estado_revision' => 'PENDIENTE']);
                }
            }
        }
        ...
        foreach ($vigentes as $v) {
            ...
            if (!$existe) {
                $carpeta->trabajadoresVerificados()->create([
                    ...
                    'estado_revision'           => (function() use ($v, $carpeta) {
                        $carpetaDate = \Carbon\Carbon::create($carpeta->anio, $carpeta->mes, 1)->startOfMonth();
                        $referenciaDate = $v->fecha_finiquito ?: $v->fecha_desactivacion ?: now();
                        $refCarbon = \Carbon\Carbon::parse($referenciaDate);
                        $refId = ($refCarbon->year * 100) + $refCarbon->month;
                        ...
                        // Si la carpeta es anterior al movimiento (Mes/Año), ignoramos el motivo de salida (estaba activo)
                        if ($folderId < $refId) {
                            return 'PENDIENTE';
                        }

                        if (in_array($v->motivo_desactivacion, ['FINIQUITADO', 'CESACION_PRINCIPAL', 'RECONOCIMIENTO_ANTIGUEDAD'])) return $v->motivo_desactivacion;
                        if (str_contains($v->motivo_desactivacion ?? '', 'PRESENTE EN OTRA')) return 'PRESENTE_OTRA_VINCULACION';
                        return 'PENDIENTE';
                    })(),
                ]);
            }
    ...
    public function cambiarEstadoTrabajadorPeriodo($id, $nuevoEstado, $destinoId = null)
    {
        ...
        // ── CASO ESPECIAL: PRESENTE EN OTRA VINCULACIÓN (Solo anula LA vinculación actual, no todas) ──
        if ($nuevoEstado === 'PRESENTE_OTRA_VINCULACION') {
            $reg->vinculacion->update([
                'is_active' => false,
                'fecha_desactivacion' => now(),
                'motivo_desactivacion' => 'PRESENTE EN OTRA VINCULACIÓN: ID ' . ($destinoId ?? 'N/A'),
            ]);
        }
        ...
        // REVERSIÓN CASO ESPECIAL: PRESENTE EN OTRA -> VOLVER A ACTIVO
        if ($estadoPrevio === 'PRESENTE_OTRA_VINCULACION' && $nuevoEstado === 'PENDIENTE') {
            $reg->vinculacion->update([
                'is_active' => true,
    ...
    private function propagarDesvinculacion(\App\Models\CarpetaVerificacionTrabajador $reg, string $nuevoEstado): void
    {
        ...
        \Illuminate\Support\Facades\Log::info('[DESVINCULACION Legacy] Propagando', [
            'nuevo_estado'        => $nuevoEstado,
            'trabajador_id'       => $trabajadorId,
            'vinculacion_ids'     => $vinculacionIds->toArray(),
            'carpetas_same_period'=> $carpetasSamePeriod->toArray(),
            'carpeta_anio'        => $reg->carpeta->anio,
            'carpeta_mes'         => $reg->carpeta->mes,
            'reg_id'              => $reg->id,
        ]);

        // 1. Marcar el nuevo estado en todas las nóminas del período
        $affected = \App\Models\CarpetaVerificacionTrabajador::whereIn('trabajador_vinculacion_id', $vinculacionIds)
            ->whereIn('carpeta_verificacion_id', $carpetasSamePeriod)
            ->where('id', '!=', $reg->id)
            ->update(['estado_revision' => $nuevoEstado]);

        \Illuminate\Support\Facades\Log::info('[DESVINCULACION Legacy] Filas actualizadas en otras nóminas: ' . $affected);
    ...
    /**
     * Reversión: si el contratista cambia de estado de desvinculación → ACTIVO,
    ...
    public function actualizarFechaFiniquito($id, $fecha)
    {
        ...
        if ($reg && $reg->vinculacion) {
            ...
            // Si el trabajador fue finiquitado (o cesado / con antiguedad reconocida), sincronizar fecha en todas sus vinculaciones
